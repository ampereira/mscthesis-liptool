...
void var(TLorentzVector* v)	{
	TRandom r(time(NULL));
	v->SetPxPyPzE(
			r.Gaus(v->Px(),0.00*v->Px()),
			r.Gaus(v->Py(),0.00*v->Py()),
			r.Gaus(v->Pz(),0.00*v->Pz()),
			r.Gaus(v->E(), 0.00*v->E() )
			);
}
...
void printArgs(double t_mass[], double w_mass[], 
		double in_mpx[], double in_mpy[], double in_mpz[],
		TLorentzVector* lep_a, TLorentzVector* lep_b, 
		TLorentzVector* bl_a, TLorentzVector* bl_b, bool last)
{

	static ofstream args( "output/dilepArgs");

	args << endl;
	args << "Dilep args" << endl;
	args << t_mass[0] << "; " << w_mass[0] << "; " << endl
		 << in_mpx[0] << "; " << in_mpx[1] << "; "
		 << in_mpy[0] << "; " << in_mpy[1] << "; "
		 << in_mpz[0] << "; " << in_mpz[1] << "; " << endl
		 << lep_a->Px() << "; " << lep_a->Py() << "; " << lep_a->Pz() << "; "
		 	<< lep_a->E() << "; " << endl
		 << lep_b->Px() << "; " << lep_b->Py() << "; " << lep_b->Pz() << "; "
		 	<< lep_b->E() << "; " << endl
		 << bl_a->Px() << "; " << bl_a->Py() << "; " << bl_a->Pz() << "; "
		 	<< bl_a->E() << "; " << endl
		 << bl_b->Px() << "; " << bl_b->Py() << "; " << bl_b->Pz() << "; "
		 	<< bl_b->E() << "; " << endl;

		 	printf("\n\n%lf; %lf; %lf; %lf;\n", lep_a->Px(), lep_a->Py(), lep_a->Pz(), lep_a->E());
		 	printf("%lf; %lf; %lf; %lf;\n", lep_b->Px(), lep_b->Py(), lep_b->Pz(), lep_b->E());
		 	printf("%lf; %lf; %lf; %lf;\n", bl_a->Px(), bl_a->Py(), bl_a->Pz(), bl_a->E());
		 	printf("%lf; %lf; %lf; %lf;\n", bl_b->Px(), bl_b->Py(), bl_b->Pz(), bl_b->E());
}
...
void dilep(double t_mass[], double w_mass[], 
		double in_mpx[], double in_mpy[], double in_mpz[],
		TLorentzVector* lep_a, TLorentzVector* lep_b, 
//		TLorentzVector* bl_a, TLorentzVector* bl_b, bool last)
		TLorentzVector* bl_a, TLorentzVector* bl_b,
		std::vector<myvector>* neutrinoContainer)
{

	//printArgs(t_mass,w_mass,in_mpx,in_mpy,in_mpz,lep_a,lep_b,bl_a,bl_b,last);
...
//	vector<myvector> *neutrinoContainer;
//	if(last)
//		neutrinoContainer = new std::vector<myvector>; //// must use new
	...
	for (int j=0; j<8; j+=2){
		...
		if ( output[j+1]==0 && delta>=0) {
			...
			/*
			TLorentzVector m_w1(
				rec_x1+lep_a->Px(), rec_y1+lep_a->Py(),
				rec_z1+lep_a->Pz(), rec_e1+lep_a->E());
			TLorentzVector m_w2(
				rec_x2+lep_b->Px(), rec_y2+lep_b->Py(),
				rec_z2+lep_b->Pz(), rec_e2+lep_b->E());
			TLorentzVector m_t1(
				rec_x1+ bl_a->Px(), rec_y1+ bl_a->Py(),
				rec_z1+ bl_a->Pz(), rec_e1+ bl_a->E());
			TLorentzVector m_t2(
				rec_x2+ bl_b->Px(), rec_y2+ bl_b->Py(),
				rec_z2+ bl_b->Pz(), rec_e2+ bl_b->E());
			*/
			...
			if ( m_good_eq1 && m_good_eq2 && m_good_eq3 && m_good_eq4 && m_good_eq5 && m_good_eq6 ) {
				//cout << endl << "rec_x1:" << rec_x1 << "\trec_y1:" << rec_y1 << " \trec_z1:" << rec_z1 << "\trec_z2:" << rec_z2 << endl;
//				if(last)
