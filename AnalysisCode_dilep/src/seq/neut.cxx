...
long long int num_measurements = 0;
...
namespace CPU {
	...
	// Wrapper for the dilep calculation
	vector<myvector>* dilep(unsigned iterations, double t_mass[], double w_mass[], 
			...
			TLorentzVector* bl_a, TLorentzVector* bl_b, int *hasSol)
	{
		...
		for (unsigned i = 0; i < iterations; ++i) {
			...
			partial_result = calc_dilep(t_mass, w_mass, in_mpx, in_mpy, in_mpz, lep_a, lep_b, bl_a, bl_b);
